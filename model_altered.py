...
class CATransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=10, shift_size=0,
        ...
        if norm_layer is not None: # my add
    ...
    def forward(self, x, xm, mask=None, img_size=(128, 128)):
        ...
        shortcut = x
        if self.norm1 is not None: # my add
            x = self.norm1(x) # my comment
        ...
        if self.norm2 is not None: # my add
            x = x + self.drop_path(self.mlp(self.norm2(x), img_size=img_size)) # my comment
...
class ShadowFormer(nn.Module):
    ...
    def forward(self, x, xm, mask=None):
        ...
        return y, output_proj # y is the corrected image, output_proj is what is added to the shadow image
