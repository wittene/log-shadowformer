...
class LogRatioBlock(nn.Module):
    def __init__(self, in_chans=4, activation1 = nn.ReLU(inplace=True), activation2 = nn.ReLU(inplace=True)):
        super(LogRatioBlock, self).__init__()
        self.conv0 = nn.Conv2d(in_chans, in_chans, kernel_size=5, padding=5//2) # In: 256x256, 3 channels, output: 251x251, 3 channels
        self.activation1 = activation1
        self.conv1 = nn.Conv2d(in_chans, in_chans, kernel_size=3, padding=3//2) # In: 249x249, 3 channels, output: 247x247, 3 channels
        self.activation2 = activation2

    def forward(self, x):
        x1 = self.conv0(x)
        y1 = self.activation1(x1)
        x2 = self.conv1(y1)
        y2 = self.activation2(x2)
        return y2
...
class ShadowFormer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
        ...
        if self.log_input:
            # Channels are: R, G, B, Mask
            self.logratio_block = LogRatioBlock(in_chans=4, activation1 = nn.ReLU(inplace=True), activation2 = nn.ReLU(inplace=True))
            # if we are using log data, we want the output to be 4 channels: gamma, r, g, b
    ...
    def forward(self, x, xm, mask=None):
        ...
        # Output Projection
        if self.log_input:
            gamma_rgb = self.output_proj(deconv2, img_size = self.img_size)
            gamma = torch.clamp(gamma_rgb, 0, 1)
            gamma = gamma_rgb[:,0:1, :, :]
            inv_gamma = torch.sub(1, gamma)
            rgb = gamma_rgb[:,1:, :, :]
            op = torch.multiply(inv_gamma, rgb)
            op = torch.multiply(op, MAX_LOG_VAL)
            y = op + x
