...
def load_img(filepath, divisor, linear_transform=False, log_transform=False):
    img = cv2.cvtColor(cv2.imread(filepath, cv2.IMREAD_UNCHANGED), cv2.COLOR_BGR2RGB)
    # print(f"MIN AND MAX OF TRAIN IMG: {np.min(img)}, {np.max(img)}")
    # img = cv2.resize(img, [256, 256], interpolation=cv2.INTER_AREA)
    img = img.astype(np.float32)
    img = img/divisor
    if linear_transform:
        img = srgb_to_rgb(img)
    if linear_transform and log_transform:
        img *= 65535
        img[img!=0] = np.log(img[img!=0])
        img /= np.log(65535)
    if log_transform and not linear_transform:
        raise Exception("Cannot perform a log transform without a linear transform first.")
    return img

def load_val_img(filepath, divisor, linear_transform=False, log_transform=False):
    img = cv2.cvtColor(cv2.imread(filepath, cv2.IMREAD_UNCHANGED), cv2.COLOR_BGR2RGB)
    # img = cv2.resize(img, [256, 256], interpolation=cv2.INTER_AREA)
    resized_img = img.astype(np.float32)
    resized_img = resized_img/divisor
    if linear_transform:
        img = srgb_to_rgb(img)
    ...
    if log_transform and not linear_transform:
        raise Exception("Cannot perform a log transform without a linear transform first.")
    # print(f"MIN AND MAX OF TEST IMG: {np.min(img)}, {np.max(img)}")
    return resized_img
...
def load_mask(filepath):
    ...
    # contour = contour/255.
    img = img/255
    # print(f"MIN AND MAX OF MASK: {np.min(img)}, {np.max(img)}")
    return img
